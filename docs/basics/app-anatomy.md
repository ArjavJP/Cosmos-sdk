...
+++ https://https://github.com/ArjavJP/Cosmos-sdk/blob/v0.40.0-rc3/simapp/app.go#L145-L187
...
+++ https://https://github.com/ArjavJP/Cosmos-sdk/blob/v0.40.0-rc3/server/types/app.go#L48-L50
...
+++ https://https://github.com/ArjavJP/Cosmos-sdk/blob/v0.40.0-rc3/simapp/app.go#L198-L441

### InitChainer

The `InitChainer` is a function that initializes the state of the application from a genesis file (i.e. token balances of genesis accounts). It is called when the application receives the `InitChain` message from the Tendermint engine, which happens when the node is started at `appBlockHeight == 0` (i.e. on genesis). The application must set the `InitChainer` in its [constructor](#constructor-function) via the [`SetInitChainer`](https://godoc.org/https://github.com/ArjavJP/Cosmos-sdk/baseapp#BaseApp.SetInitChainer) method.
...
+++ https://https://github.com/ArjavJP/Cosmos-sdk/blob/v0.40.0-rc3/simapp/app.go#L454-L462
...
+++ https://https://github.com/ArjavJP/Cosmos-sdk/blob/v0.40.0-rc3/simapp/params/encoding.go#L9-L16
...
+++ https://https://github.com/ArjavJP/Cosmos-sdk/blob/590358652cc1cbc13872ea1659187e073ea38e75/simapp/encoding.go#L8-L19
...
+++ https://https://github.com/ArjavJP/Cosmos-sdk/blob/v0.40.0-rc3/proto/cosmos/bank/v1beta1/tx.proto#L10-L17
...
- Custom request type definitions for each query or transaction creation function that needs to be exposed. These custom request types build on the base `request` type of the Cosmos SDK:
  +++ https://https://github.com/ArjavJP/Cosmos-sdk/blob/v0.40.0-rc3/types/rest/rest.go#L62-L76
...
::: warning
A patch introduced in `go-grpc v1.34.0` made gRPC incompatible with the `gogoproto` library, making some [gRPC queries](https://https://github.com/ArjavJP/Cosmos-sdk/issues/8426) panic. As such, the SDK requires that `go-grpc <=v1.33.2` is installed in your `go.mod`.
...
Please see [issue #8392](https://https://github.com/ArjavJP/Cosmos-sdk/issues/8392) for more info.
:::
